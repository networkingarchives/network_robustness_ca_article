...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel(p("Application to test the robustness of a user-inputted network to missing data, to accompany an", a(href='https://culturalanalytics.org/article/25943-the-measure-of-the-archive-the-ro-bustness-of-network-analysis-in-early-modern-correspondence',"article published in the Journal of Cultural Analytics")),p("Upload an edge list (TSV or CSV) to calculate robustness scores."),p("This version is limited to 1000 edges and 20 runs to save on Shiny runtime. Download", a(href='https://github.com/networkingarchives/network_robustness_ca_article/tree/main/network_robustness_tool','here'), "and run locally in R to remove this restriction."), p("Edge list should have at least two columns, headed 'from' and 'to'. Additional columns (such as year or source) will be sampled separately and added to the plot."), p("The algorithm removes progressively larger random samples of edges and compares the full ranked metric to those calculated using the partial network. All calculations are on an unweighted, directed network"),
